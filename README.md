...
- Bound collect addresses: bind each wallet to its own destination once (Wallet Groups → Collect addresses, one line: `wallet:address,wallet:address`), then Collect TAO sends every wallet to its saved address
